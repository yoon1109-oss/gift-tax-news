...
- **점수** `relatedScore()` — 키워드 묶음 4개, **서로 겹치는 단어가 없어야 함**(겹치면 중복 가산):
  ...
  - `family` 모드는 별도 경로 — 주제어 `REL_FAMILY` 제목+3/본문+1, 자녀 대상 `REL_FAMILY_KID`
    제목+5/본문+2, 자녀 금융상품(`REL_FAMILY_PROD`)이 제목 주제면 +4.
    **①②는 개념당 1회만 계상**(키워드마다 더하면 '손주·자녀'류가 점수를 배로 먹음).
    `REL_FAMILY_KID`의 `아이`는 정규식으로 아이디어·아이폰 등을 배제
