...
- **`PI_POINTS`(연령 구간 상세)는 카드 4장이다 (2026-08-29 재구성)** —
  ...
  - 타일 3개는 `최다 구간` · `절반이 이 아래` · `1억 이하 누계`다 (`abFacts()`).
    **`절반이 이 아래`(`medianIdx`)는 누적 50%를 처음 넘는 구간** = 중앙값이 걸리는 칸이다.
    최다 구간만 보면 분포가 한쪽으로 긴 경우를 놓친다 — 실제로 10대는 최다가 `5천만 이하`인데
    절반 지점은 `1억 이하`다. 예전 타일이던 `10억 초과`는 미성년 구간에서 값이 0에 가까워 뺐다
  - **`중앙값`·`중앙 구간`·`한가운데 사람` 같은 말을 화면에 쓰지 말 것** (2026-08-29).
    `한가운데 사람이 … 구간에 있고`로 썼다가 "이게 무슨 말이냐"는 질문을 받았다.
    `절반이 ○○ 이하` 형태로만 쓴다
  - 한눈에 보기의 10세 미만·10대 줄은 **같은 기준(5천만·1억) 누계를 나란히** 적는다.
    10대는 절반 지점과 `1억 이하 누계`가 같은 칸이라, 둘을 그냥 이어 쓰면 같은 말이 두 번 나온다
