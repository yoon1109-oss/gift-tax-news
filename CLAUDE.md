...
| 세법 | `taxnews` / `press` | 서브탭 2종 — 세법 뉴스(`api/taxnews.js`) / 세무사회 보도자료(`api/kacta.js`) |
...
- 파이 앱과의 직접 관련도는 낮다(국세청 인사·세무조사·업계 동향 중심). 그래서 `TAX_REL_KW`로
  증여·상속·가업승계 계열을 **연관도 높음**으로 상단 분류한다(세무사회 탭과 같은 방식).
  `공제` 단독은 '장특공제'·'자체감사와 공제'처럼 증여와 무관한 기사를 끌어와 넣지 않았고,
  대신 혼인·출산(공제 신설 계열)을 포함했다 — 실측 53건 중 14건이 분류됨
