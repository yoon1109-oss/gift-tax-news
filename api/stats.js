...
const TAXB = [
  // [신고건수, 증여재산가액, 증여재산공제, 과세표준, 산출세액]
  [22168, 82693, 3238, 79279, 7964],
  [33884, 829081, 251825, 691193, 70346],
  [42894, 2866184, 1454779, 1832964, 187826],
  [49700, 6742897, 2440414, 6004776, 779788],
  [13073, 3924943, 659457, 4353204, 756528],
  [12697, 6090465, 2277494, 6169632, 1256388],
  [3873, 3111606, 575068, 4321221, 1159829],
  [845, 1164178, 95261, 1934034, 634619],
  [508, 991195, 82836, 1804285, 631196],
  [618, 5189751, 123685, 12919309, 5870443],
];
...
const NOTICE = [
  ...
  `그래도 소액 구간이 두꺼운 이유 — <b>타인 간 증여는 증여재산공제가 0원</b>이고, 친족이라도 10년 내 공제를 이미 쓴 <b>재차 증여</b>는 소액이어도 과세됩니다. 실제로 <b>1천만 이하</b> 구간도 건당 산출세액이 <b>${perOne(TAXB[0][4], TAXB[0][0])}</b>이고, 증여재산공제는 건당 ${perOne(TAXB[0][2], TAXB[0][0])}에 그칩니다 — 세금이 0원인 신고가 아닙니다. (아래 '구간별 세금' 표에서 확인)`,
  '구간 기준은 <b>증여재산가액 + 증여재산가산액</b>입니다 — 이번에 받은 금액이 아니라 <b>10년 누적</b>에 가깝습니다. (가산액 = 증여 전 10년 이내 동일인에게 받은 증여재산 합계가 1천만원 이상인 경우 그 금액)',
...
const PI_POINTS = {
  ...
  items: [
    { group: '미성년 자녀 증여 (20세 미만)', tag: '파이 타깃', lines: [
      ...
      `이 중 ${pct(cum(MINOR_BAND, 2), MINOR_TOTAL)}(${nf(cum(MINOR_BAND, 2))}명)가 <b>1억 이하</b>(10년 누적 기준) 소액 증여`,
    ...
    { group: '10세 미만 — 가장 이른 증여', tag: '', lines: [
      ...
      '구간이 <b>10년 누적</b> 기준이라, 미성년 공제한도(10년 2천만원)를 갓 넘긴 증여가 이 구간에 쌓입니다',
    ]},
    { group: '증여의 절반 이상이 소액', tag: '', lines: [
      ...
      '건수 기준으로는 대형 자산가가 아니라 생활형 증여가 다수 (금액은 10년 누적 기준)',
    ]},
...
const RAW = [
  ...
  { title: '2025년 구간별 세금 — 과세미달이 제외됐다는 근거',
    note: '단위: 백만원 · 모든 구간에서 산출세액 > 0 · 출처 국세통계 6.3.2 증여세 신고 현황Ⅱ(증여재산가액 등)',
    columns: ['구간', '신고건수', '증여재산가액', '증여재산공제', '과세표준', '산출세액', '건당 산출세액'],
    rows: BANDS.map((b, i) => {
      const [cnt, amt, ded, base, tax] = TAXB[i];
      return [b, nf(cnt), nf(amt), nf(ded), nf(base), nf(tax), perOne(tax, cnt)];
    }) },
