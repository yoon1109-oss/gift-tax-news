...
const UPDATED_AT = '2026-09-21'; // 데이터가 실제 바뀐 날 (정리 기준)
const CHECKED_AT = '2026-09-21';
// 친구 초대는 계좌 이벤트와 점검 시점이 다르다. 한 값으로 묶으면 한쪽이 실제보다
// 최신인 것처럼 보이므로 따로 둔다.
const REFERRAL_CHECKED_AT = '2026-09-21'; // 마지막으로 재조사·확인한 날 (변경 없어도 갱신)

// 업데이트 메모 — 이벤트 '내용이 실제로 바뀐 것'만 최신순으로 기록 (배치가 변경 시 맨 앞에 추가)
const CHANGES = [
  { date: '2026-09-21', text: '한국투자증권 주식모으기 시즌2의 종료일이 10월 3일로 확인돼 바로잡았어요(기존 10월 8일). 부모와 자녀가 함께 참여하면 100쌍(200명)을 추첨해 각각 1만원을 드려요' },
...
const EVENTS = [
  ...
  },
  {
    ...
    benefits: [
      ...
      '2026.07.16 이후 최초 개설한 미성년 신규고객 대상 (법인·외국인·임직원·성인 제외)',
      '2026.09.30까지 비대면 위탁종합계좌를 보유하고 있어야 지급',
    ],
    domesticFee: '',
    usFee: '',
  ...
  },
  {
    ...
    benefits: [
      '스마트금융센터(비대면) 신규 미성년 계좌 개설 + 순입금 시 최대 5만원 현금 & 평생 수수료 우대',
      '순입금 조건 충족 후 국내주식 1억원 이상 거래 시 5만원 추가 (합산 최대 10만원)',
      '이벤트 기간 내 1천만원 이상 주식거래 필수, 2026.10.31까지 순입금액 이상 자산 유지',
      '부모 명의 SK증권 계좌 보유 필수',
    ],
    domesticFee: '',
    usFee: '',
...
const TAX_CHECKED_AT = '2026-09-21';
const TAX_ACCOUNTS = [
  {
    broker: '하나증권',
    title: '개인연금 순증 이벤트',
